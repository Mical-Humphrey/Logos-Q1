...
def test_run_backtest_generates_metrics_and_artifacts():
    ...
    result = run_backtest(prices=prices, signals=signals, asset_class="equity", periods_per_year=252)

    assert set(result.keys()) == {"equity_curve", "positions", "returns", "trades", "metrics"}
    ...
    flat_result = run_backtest(prices=prices, signals=flat_signals, asset_class="equity", periods_per_year=252)
