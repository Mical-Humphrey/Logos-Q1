...
class FeatureStore:
     ...
     def register(
         ...
         *,
         ...
         return FeatureVersion(name=name, version=version, path=data_path, metadata_path=meta_path)

     def load(self, name: str, version: str | None = None) -> tuple[pd.DataFrame, dict[str, Any]]:
     ...
     def latest_version(self, name: str) -> FeatureVersion:
         ...
         metadata = json.loads(meta_path.read_text(encoding="utf-8"))
         return FeatureVersion(name=name, version=metadata["version"], path=data_path, metadata_path=meta_path)
