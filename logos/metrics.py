...
def sharpe(returns: pd.Series, risk_free_rate: float = 0.0, periods_per_year: int = TRADING_DAYS) -> float:
    r = _clean_returns(returns)
    if r.empty:
        return 0.0
...
def sortino(returns: pd.Series, risk_free_rate: float = 0.0, periods_per_year: int = TRADING_DAYS) -> float:
    r = _clean_returns(returns)
    if r.empty:
        return 0.0
    downside = r.copy()
